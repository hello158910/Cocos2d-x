...
void StaticDynamic::readSceneFile(const char *csbfilename)
{
	...
	//auto triSprite = (Sprite *)csbRoot->getChildByName("triangle1_01_0");
	...
	//fixtureDef.shape = &seesawShape;
	//fixtureDef.density = 5.0f;
	//fixtureDef.friction = 0.1f;
	//fixtureDef.restitution = 0.1f;
	//auto seesawSprite = (Sprite *)csbRoot->getChildByName("ployblock1_02_0");
	//bodyDef.userData = seesawSprite;
	//Size ts = seesawSprite->getContentSize();
	//Point loc = seesawSprite->getPosition();
	//float scaleX = seesawSprite->getScaleX();	// 對矩形圖示 X 軸縮放值
	//float scaleY = seesawSprite->getScaleY();	// 對矩形圖示 Y 軸縮放值
	//bodyDef.position.Set(loc.x / PTM_RATIO, loc.y/ PTM_RATIO);
	//b2Body *seesawBody = _b2World->CreateBody(&bodyDef);
	//
	//// 算出 seesawBoard 的縮放後的寬高, 4 為預留的寬度，不跟其他的圖片重疊
	//float bw = (ts.width - 4)* scaleX;
	//float bh = (ts.height - 4)* scaleY;
	//// 設定剛體的範圍是一個 BOX （可以縮放成矩形）
	//seesawShape.SetAsBox(bw*0.5f / PTM_RATIO, bh*0.5f / PTM_RATIO);
	//seesawBody->CreateFixture(&fixtureDef);
